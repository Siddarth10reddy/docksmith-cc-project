...
# ── User-namespace pre-check ──────────────────────────────────────────────────
APPARMOR_NS_RESTRICT="/proc/sys/kernel/apparmor_restrict_unprivileged_userns"
UNPRIV_NS_CLONE="/proc/sys/kernel/unprivileged_userns_clone"

if [ -f "$APPARMOR_NS_RESTRICT" ] && [ "$(cat $APPARMOR_NS_RESTRICT)" = "1" ]; then
    echo ""
    echo -e "  ${YELLOW}NOTE: AppArmor restricts unprivileged user namespaces.${NC}"
    echo -e "  ${YELLOW}Disabling for this session (requires sudo)...${NC}"
    sudo sysctl -w kernel.apparmor_restrict_unprivileged_userns=0 >/dev/null
    ok "User namespaces enabled for this session."
    echo -e "  (To make permanent: add 'kernel.apparmor_restrict_unprivileged_userns=0' to /etc/sysctl.conf)"
elif [ -f "$UNPRIV_NS_CLONE" ] && [ "$(cat $UNPRIV_NS_CLONE)" = "0" ]; then
    echo ""
    echo -e "  ${YELLOW}NOTE: Unprivileged user namespaces are disabled.${NC}"
    echo -e "  ${YELLOW}Enabling for this session (requires sudo)...${NC}"
